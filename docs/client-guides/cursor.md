...
1. Runner 配置：`{"B": {"type": "manual"}}`，`agent-dealer watch` 会把接力提示词打印出来。
2. 把提示词粘贴到 Cursor 的 Agent 对话中（确保 Cursor 打开了共享目录作为工作区）。
3. Cursor 中的模型按提示读取 SKILL.md、恢复状态、执行并用 `agent-dealer publish` 发布事件
   （Cursor 内终端可直接运行 `agent-dealer`，无需额外 API key）。
