...
子命令：
  ...
  validate  校验任务（等价 python -m agent_collaboration.validator）
  watch     Runner 监听调度循环
"""
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    print("  下一步: collab next %s" % task_dir)
    return 0
...
def cmd_event_prepare(args: argparse.Namespace) -> int:
    ...
    print("  预校验: collab publish --dry-run %s %s" % (args.task_dir, out_path))
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    def add_actor_flags(sp: argparse.ArgumentParser, role_default: str = "B") -> None:
        ...
        sp.add_argument("--client", default=os.environ.get("MMAC_CLIENT", "collab-cli"))
