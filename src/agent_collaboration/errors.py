...
E101_INVALID_STATE = _reg(
    ...
    "用 `collab status <task>` 查看当前状态")
E102_INVALID_EVENT = _reg(
    "MMAC-E102_INVALID_EVENT", "事件结构非法",
    "运行 `collab doctor <task>` 定位字段问题；参照 references/event-schema.md")
...
E105_INVALID_CONTROL = _reg(
    ...
    "运行 `collab doctor <task>`；对照 references/control-schema.md 修复")
...
E302_ARTIFACT_MISSING = _reg(
    ...
    "确认路径相对任务目录（任务目录外用绝对路径）；用 `collab artifact add` 固化产物")
...
E601_RUNNER = _reg(
    ...
    "检查 adapters 配置与客户端可用性：`collab doctor <task>`")
