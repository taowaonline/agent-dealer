...
> 本文件只保留 Agent 必须执行的工作流；发布与校验一律使用 `agent-dealer`（或 `python -m agent_dealer`）。
...
- **启动前**：`agent-dealer models` 探测本机已安装的模型客户端（claude/codex/kimi/
...
- **单会话（solo）**：`agent-dealer init <task-id> --solo` —— 一个客户端/模型扮演
...
任务收尾用 `agent-dealer report tasks/<task-id>` 输出任务报告：各 agent 的事件与
